...
List filters include `child`, `name`, `dosage_unit`, `tags` (comma-separated
tag names), and `date` / `date_min` / `date_max`. Despite the `date*` names,
those filters are ISO 8601 **datetime** filters on the `time` field (same
pattern as other timed endpoints), for example `2020-03-12T14:30:00-07:00` —
not bare `YYYY-MM-DD` dates.
